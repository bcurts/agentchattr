#!/usr/bin/env sh
# agentchattr — starts server (if not running) + OrcaRouter API agent wrapper
# Usage: sh start_orca.sh
...
if [ -z "$ORCAROUTER_API_KEY" ]; then
    ...
    echo "Then: export ORCAROUTER_API_KEY=your-key-here"
    exit 1
fi
...
.venv/bin/python wrapper_api.py orca
